/* t-gpg.c - Regression test.
   Copyright (C) 2016 Seiya Kawashima <skawashima@uchicago.edu>
...
main (int argc, char **argv)
{
  ...
  /* Set '--log-file' flag through environment variable. */
  char *log_file = "/tmp/gpg-log-file.txt";
  setenv("GPG_LOG_FILE",log_file,1);
  /* Set '--debug-level' flag through environment variable. */
  setenv("GPG_DEBUG_LEVEL","guru",1);
  /* Set '--pinentry-user-data' flag to pass user data as
     PINENTRY_USER_DATA to pinentry. */
  setenv("PINENTRY_USER_DATA","msg=helloWorld",1);
